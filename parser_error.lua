...
while index ~= nil do
    ...
    while parserCellNumber<=30 do
        ...
        if index == nil then
            ...
            errorField_0 = tonumber((string.sub(filtered_data_payload, 0, 4)), 16)    -- (2 bytes) extracting the Cell Index, that tells which Cell's voltages are present in the CAN message 
            if errorField_0 == nil then errorField_0 = 0 end                        -- handling nil value
            -- print(data_cellIndex)                                                    -- DEBUG MESSAGE
            errorField_1 = tonumber((string.sub(filtered_data_payload, 5, 8)), 16)   -- (2 bytes) extracting voltages of Cell number (cell_index+3)
            if errorField_1 == nil then errorField_1 = 0 end                      -- handling nil value
            -- print(data_cellIndex0)                                                   -- DEBUG MESSAGE
            errorField_2 = tonumber((string.sub(filtered_data_payload, 9, 12)), 16)  -- (2 bytes) extracting voltages of Cell number (cell_index+3)
            if errorField_2 == nil then errorField_2 = 0 end                      -- handling nil value
            -- print(data_cellIndex1)                                                   -- DEBUG MESSAGE
            errorField_3 = tonumber((string.sub(filtered_data_payload, 13, 16)), 16) -- (2 bytes) extracting voltages of Cell number (cell_index+3)
            if errorField_3 == nil then errorField_3 = 0 end                      -- handling nil value
            -- print(data_cellIndex2)                                                   -- DEBUG MESSAGE


            error_0_Id = ((errorField_0 & 64512) >> 10)
            error_0_State = ((errorField_0 & 896) >> 7)

            error_1_Id = ((errorField_1 & 64512) >> 10)
            error_1_State = ((errorField_1 & 896) >> 7)

            error_2_Id = ((errorField_2 & 64512) >> 10)
            error_2_State = ((errorField_2 & 896) >> 7)

            error_3_Id = ((errorField_3 & 64512) >> 10)
            error_3_State = ((errorField_3 & 896) >> 7)
